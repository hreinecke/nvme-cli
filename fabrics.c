...
int fabrics_discover(const char *desc, int argc, char **argv, bool connect)
{
	...
	memset(&static_host, 0, sizeof(static_host));
	...
	if (!static_port.transport && !strcmp(static_port.traddr, "none")) {
...
int fabrics_connect(const char *desc, int argc, char **argv)
{
	...
	memset(&static_host, 0, sizeof(static_host));
	memset(&static_subsys, 0, sizeof(static_subsys));
