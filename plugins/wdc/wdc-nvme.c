				    ...
				    void **cbs_data)
{
	...
	*cbs_data = NULL;
	...
	ret = nvme_get_log(fd, lid, 0xFFFFFFFF, NVME_LOG_LSP_NONE, 0, 0, false, uuid_ix, 0, le32_to_cpu(hdr_ptr->length), data);
	if (ret) {
...
static int wdc_do_cap_telemetry_log(int fd, char *file, __u32 bs, int type, int data_area)
{
	...
	if (type == WDC_TELEMETRY_TYPE_HOST) {
		...
		err = nvme_get_features(fd, WDC_VU_DISABLE_CNTLR_TELEMETRY_OPTION_FEATURE_ID, 0, 0, 0,
				0, 4, buf, &result);
		if (err == 0) {
			if (result == 0) {
				/* enabled */
...
static int wdc_do_clear_pcie_correctable_errors_fid(int fd)
{
	...
	ret = nvme_set_features(fd, WDC_NVME_CLEAR_PCIE_CORR_FEATURE_ID, 0,
				value, 0, 0, 0, 0, 0, NULL, &result);
...
static int wdc_do_clear_fw_activate_history_fid(int fd)
{
	...
	ret = nvme_set_features(fd, WDC_NVME_CLEAR_FW_ACT_HIST_VU_FID, value, 0,
				0, 0, 0, 0, 0, NULL, &result);
	...
	if (cfg.disable) {
		...
		if (cfg.enable) {
			ret = nvme_set_features(fd, WDC_VU_DISABLE_CNTLR_TELEMETRY_OPTION_FEATURE_ID, 0, 0,
						9, 0, 0, 0, 0, buf, &result);
	   }
	   else if (cfg.status) {
		   ret = nvme_get_features(fd, WDC_VU_DISABLE_CNTLR_TELEMETRY_OPTION_FEATURE_ID, 0, 0, 0,
					0, 4, buf, &result);
			if (ret == 0) {
				if (result)
...
static int wdc_do_drive_essentials(int fd, char *dir, char *key)
{
	...
	for (listIdx = 1; listIdx < (sizeof(deFeatureIdList) / sizeof(deFeatureIdList[0])); listIdx++)
	{
		...
		if (deFeatureIdList[listIdx].featureId == FID_LBA_RANGE_TYPE)
			continue;
		ret = nvme_get_features(fd, deFeatureIdList[listIdx].featureId, WDC_DE_GLOBAL_NSID, FS_CURRENT, 0,
		...
		struct plugin *plugin)
{
	...
	ret = wdc_do_drive_essentials(fd, d_ptr, k);
	nvme_free_tree(r);
	return ret;
}
		...
		struct command *command, struct plugin *plugin)
{
	...
	nvme_get_features(fd, 0x10, 0, 0, 0, 0, 0, 0, &hctm_tmt);
