...
__public struct nvme_fabrics_config *nvmf_context_get_fabrics_config(struct nvmf_context *fctx)
{
	return &fctx->cfg;
}
		...
		bool *discover, nvme_ctrl_t *cp)
{
	...
	if (ret == EINVAL && fctx->cfg.disable_sqflow) {
		...
		struct nvme_ctrl **ctrl)
{
	...
	ret = nvme_add_ctrl(fctx, h, c, &fctx->cfg);
	if (ret) {
		nvme_free_ctrl(c);
		return ret;
	}
